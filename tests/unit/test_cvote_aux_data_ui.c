...
static const uint8_t CVOTE_UI_TEST_DELEGATION_KEY_PATH_INVALID_CHAIN_PAYLOAD[] = {
    ...
    0x02, 0x05,
    0x80, 0x00, 0x06, 0x9E,   // 1694' (PURPOSE_CVOTE_KEY | HARDENED)
    0x80, 0x00, 0x07, 0x17,   // 1815' (ADA_COIN_TYPE | HARDENED)
    0x80, 0x00, 0x00, 0x00,   // 0' (account)
    0x00, 0x00, 0x00, 0x01,   // 1 (chain) — fails bip44_isCVoteKeyPath (must be 0)
    0x00, 0x00, 0x00, 0x00,   // 0 (address_index)
...
int main(void) {
    const struct CMUnitTest tests[] = {
        cmocka_unit_test(
            test_cvote_delegation_ui_status_remains_success_after_each_delegation),
        cmocka_unit_test(
            test_cvote_delegation_page_starts_on_new_page_with_correct_pair_count),
        cmocka_unit_test(

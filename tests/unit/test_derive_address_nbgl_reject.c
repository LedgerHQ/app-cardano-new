...
// REWARD_KEY (0x0E) + network 0x03 + no payment part + STAKING_PART_KEY_HASH (0x33) + 28 zero bytes.
// policyForShowDeriveAddress denies REWARD_KEY unless staking part is KEY_PATH.
...
static void test_derive_address_display_policy_deny(void **state) {
    ...
    test_read_buffer_t buf = make_test_read_buffer(
        REWARD_KEY_HASH_STAKING_DISPLAY_APDU,

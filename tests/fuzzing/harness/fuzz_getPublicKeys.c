 ...
 *
 * This harness feeds random APDU commands to the getPublicKeys handler,
 * testing the BIP44 path parsing, key derivation, and response formatting
 ...
 *   P2  = 0x00 (unused)
 *   LC  = variable length
 *   DATA = BIP44 path and parameters
 */
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    // The dispatcher validates CLA, INS, P1/P2 and calls handler_get_public_key
    apdu_dispatcher(&cmd);

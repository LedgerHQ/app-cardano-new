...
static void cvote_extract_pubkey(const cvote_credential_t *credential,
                                 uint8_t *out_pubkey,
    ...
    switch (credential->type) {
        ...
        default:
            LEDGER_ASSERT(false,
                          "Invalid CVote credential type %u (only 0=KEY, 2=KEY_PATH allowed)",
                          credential->type);
    }
    // LCOV_EXCL_STOP
}
...
static void cvote_hash_builder_add_vote_key(cvote_aux_data_t *aux_data) {
    ...
    switch (aux_data->format) {
        ...
        case CIP36:
            ...
            should_add_vote_key = (aux_data->hash_builder.state !=
                                   AUX_DATA_HASH_BUILDER_IN_CVOTE_REGISTRATION_PAYLOAD_DELEGATIONS);
            break;
        // LCOV_EXCL_START
        default:
            ASSERT(false);
    ...
    if (should_add_vote_key) {
        ...
        auxDataHashBuilder_cVoteRegistration_addVoteKey(&aux_data->hash_builder,
                                                        pubkey,
                                                        sizeof(pubkey));
    }
}
...
static void cvote_hash_builder_add_staking_key(cvote_aux_data_t *aux_data) {
    ...
    auxDataHashBuilder_cVoteRegistration_addStakingKey(&aux_data->hash_builder,
                                                       pubkey,
                                                       sizeof(pubkey));
}
...
static void cvote_hash_builder_add_common_fields(cvote_aux_data_t *aux_data) {
    ASSERT(aux_data != NULL);
    LEDGER_ASSERT(!aux_data->final_fields_processed,
                  "CVote common fields already processed");  // LCOV_EXCL_LINE
    ...
    if (aux_data->format == CIP36) {
        auxDataHashBuilder_cVoteRegistration_addVotingPurpose(&aux_data->hash_builder,
...
static void cvote_append_registration_signature(cvote_aux_data_t *aux_data) {
    ...
    auxDataHashBuilder_cVoteRegistration_finalizePayload(&aux_data->hash_builder,
                                                         payload_hash,
                                                         sizeof(payload_hash));
    ...
    auxDataHashBuilder_cVoteRegistration_addSignature(&aux_data->hash_builder,
                                                      aux_data->registration_signature,
                                                      sizeof(aux_data->registration_signature));

...
void addPaymentInfoUIPairs(const address_params_t *address_params) {
    ...
    switch (determinePaymentChoice(address_params->type)) {
        case PAYMENT_PATH: {
            ...
            UI_ADD_FORMAT1(UI_LABEL_BY_SCREEN("Payment key path", "Pay path"),
                           MAX_BIP44_PATH_STRING_LENGTH,
                           format_bip44_path,
                           &address_params->paymentKeyPath);
            break;
        }

        case PAYMENT_SCRIPT_HASH: {
            LEDGER_ASSERT(
                addressParams_getPaymentPartType(address_params) == PAYMENT_PART_SCRIPT_HASH,
                "Payment credential must be SCRIPT_HASH");
            ASSERT(address_params->paymentScriptHash != NULL);
            UI_ADD_FORMAT3(UI_LABEL_BY_SCREEN("Payment script hash", "Pay script"),
                           MAX_BECH32_STRING_LENGTH,
                           format_bech32,
                           BECH32_PREFIX_SCRIPT_HASH,
                           address_params->paymentScriptHash,
                           SCRIPT_HASH_LENGTH);
            break;
        }
...
void addStakingInfoUIPairs(const address_params_t *address_params) {
    ...
    switch (addressParams_getStakingPartType(address_params)) {
            ...
            switch (address_params->type) {
                case BYRON:
                    UI_ADD_STATIC(UI_STATIC_LABEL("Warning:"),
                                  UI_LABEL_BY_SCREEN("Legacy Byron address (no staking rewards)",
                                                     "Byron (no staking)"));
                    break;
        ...
        case STAKING_PART_KEY_PATH: {
            ...
            UI_ADD_FORMAT1(UI_STATIC_LABEL("Staking path"),
                           MAX_BIP44_PATH_STRING_LENGTH,
                           format_bip44_path,
                           &address_params->stakingKeyPath);
            break;
        }
        ...
        case STAKING_PART_KEY_HASH: {
            ...
            UI_ADD_FORMAT3(UI_LABEL_BY_SCREEN("Stake key hash", "Stake key"),
                           MAX_BECH32_STRING_LENGTH,
                           format_bech32,
                           BECH32_PREFIX_STAKE_KEY_HASH,
                           address_params->stakingKeyHash,
        ...
        case STAKING_PART_SCRIPT_HASH: {
            LEDGER_ASSERT(
                addressParams_getStakingPartType(address_params) == STAKING_PART_SCRIPT_HASH,
                "Staking credential must be SCRIPT_HASH");
            ASSERT(address_params->stakingScriptHash != NULL);
            UI_ADD_FORMAT3(UI_LABEL_BY_SCREEN("Stake script hash", "Stake hash"),
                           MAX_BECH32_STRING_LENGTH,
                           format_bech32,
                           BECH32_PREFIX_SCRIPT_HASH,
                           address_params->stakingScriptHash,
                           SCRIPT_HASH_LENGTH);
            break;
        }

        case STAKING_PART_BLOCKCHAIN_POINTER: {
            UI_ADD_FORMAT1(UI_LABEL_BY_SCREEN("Stake key pointer", "Stake ptr"),
                           MAX_BIP44_PATH_STRING_LENGTH,
                           format_blockchain_pointer,
                           address_params->stakingKeyBlockchainPointer);
            break;
        }
